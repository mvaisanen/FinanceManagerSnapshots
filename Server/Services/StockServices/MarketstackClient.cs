...
{
    public class MarketstackClient : IDisposable
    {
        private Logger _log;
        public readonly int InstanceNumber;
        private static int InstancesCreated;
        private HttpClient _client;
        public static readonly string ClientName = "Marketstack";

        public MarketstackClient(HttpClient httpClient)
        {
            _log = LogManager.GetCurrentClassLogger();
            InstanceNumber = Interlocked.Increment(ref InstancesCreated);
            _log.Debug($"MarketstackClient #{InstanceNumber} created");
            _client = httpClient;
        }
